...
export default function Sidebar() {
  ...
  if (user?.role === 'admin') links.push({ to: '/admin', icon: Shield, label: t('admin') });

  return (
    <>
      {/* Mobile hamburger */}
      <button onClick={() => setIsOpen(true)} className="md:hidden fixed top-4 left-4 z-50 w-10 h-10 bg-[#111] border border-[#1E1E1E] rounded-xl flex items-center justify-center text-white hover:border-[#FF6B35] transition-all">
        <Menu size={20} />
      </button>

      {/* Overlay */}
      {isOpen && <div className="md:hidden fixed inset-0 bg-black/70 z-40" onClick={() => setIsOpen(false)} />}

      {/* Sidebar */}
      <aside className={`fixed top-0 h-full w-64 bg-[#0D0D0D] border-r border-[#1E1E1E] flex flex-col z-50 transition-transform duration-300 ${isOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0`}>
        
        {/* Logo */}
        <div className="p-6 border-b border-[#1E1E1E] flex items-center justify-between">
          <img src="/PumP_logo.png" alt="PumP" className="h-12 object-contain" />
          <button onClick={() => setIsOpen(false)} className="md:hidden text-[#888] hover:text-white"><X size={22} /></button>
        </div>
        ...
        <div className="p-4 border-b border-[#1E1E1E]">
          <div className="flex items-center gap-3">
            {user?.avatar ? (
              ...
              <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#FF6B35] to-[#FF4500] flex items-center justify-center text-white font-bold text-lg flex-shrink-0">
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
          {links.map(({ to, icon: Icon, label }) => (
            <NavLink key={to} to={to} onClick={() => setIsOpen(false)} className={({ isActive }) =>
              `flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all ${isActive ? 'bg-gradient-to-r from-[#FF6B35] to-[#FF4500] text-white' : 'text-[#888] hover:text-white hover:bg-[#1A1A1A]'}`
            }>
        ...
        <div className="p-4 border-t border-[#1E1E1E] space-y-2">
          <button onClick={toggleLang} className="flex items-center gap-3 px-4 py-3 rounded-xl text-[#888] hover:text-white hover:bg-[#1A1A1A] w-full text-sm font-medium transition-all">
            <Globe size={18} /><span>{lang === 'he' ? 'English' : 'עברית'}</span>
          </button>
